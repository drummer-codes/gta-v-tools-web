...
function search() {
    ...
    $.get('?search', args, function (response) {
        ...
        if (Page == 1) {
            ...
            <div class="card d-block m-4 bg-dark2 text-white">
                <div class="card-body text-center">
                    ...
                    <br class="d-block d-md-none">
                    <span class="mx-4"><span class="text-bm font-weight-bold">${numberWithSpaces(response.count.total)}</span> <span class="ms-2">Animations</span></span>
                </div>
            </div>
            `);
        }
            ...
            <tr>
                ...
                <td${border} class="copy text-bm">
                    ...
                    <div class="ingame-trigger">
                        <a href="#" class="btn btn-sm btn-success" data-ingame="anim:${x.dict},${x.name}"><i class="fas fa-play-circle"></i> In-Game</a>    
                    </div>
                </td>
        ...
        <div class="card bg-dark mt-4" id="bottom">
            <div class="card-body">
                <p>Showing <span class="font-weight-bold">${shown}</span> / ${response.count.total} animations</p>
...
function moreparams() {
    return {
        p: $('#peds').is(':checked') ? 1 : null,
    };
}
$('#peds').prop('checked', params.p == undefined ? false : params.p == 1);
$(document).on('click', '#peds', function () {
    Page = 1;
    search();
});