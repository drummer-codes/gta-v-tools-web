...
if (isset($_GET['search'])) {
    ...
    $sqlc = "SELECT COUNT(*) AS `total` FROM `gta_tools_postfx` $where";
    ...
    $sql = "SELECT * FROM `gta_tools_postfx` $where $order LIMIT $per_page OFFSET $offset";
    $data = DB::query($sql);
...
function update()
{
    ...
    $html .= '<tbody>';
    $info = [];
    $json = json_decode(file_get_contents('https://raw.githubusercontent.com/DurtyFree/gta-v-data-dumps/master/animPostFxNamesCompact.json'), true);
    foreach ($json['Names'] as $x) {
        DB::query("DELETE FROM `gta_tools_postfx` WHERE `name`='$x'");
        DB::insert('postfx', [
            'name' => $x,
            'url' => $x . '.jpg',
        ]);
...
function raw()
{
    $data = DB::query("SELECT * FROM `gta_tools_postfx` ORDER BY `name`");
    $count = count($data);
    $html = '
        <div class="card d-block m-4 bg-dark text-light">
            <div class="card-body text-center">
        ...
        ';
    $html .= "<table class='table table-dark table-striped table-hover text-center' style='margin-top: 3em;'>";
    $html .= "<thead class='thead-dark'><tr><th scope='col'>Name</th><th scope='col'></th><tr></thead>";
    $html .= '<tbody>';
    foreach ($data as $x) {
        $html .= '
        <tr>
